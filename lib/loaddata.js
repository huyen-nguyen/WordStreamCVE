...
function loadCVE(draw, space) {
    var rawData = JSON.parse(cve);
    console.log("raw data:");
    console.log(JSON.parse(JSON.stringify(rawData)));
    var keys = [];
    keys = d3.keys(rawData[0]["words"]);
    ...
    rawData.forEach(month => {
        ...
        keys.forEach(level => {
            ...
            var temp = JSON.parse(JSON.stringify(month["words"][level]));
            delete month["words"][level];
            freq.push(temp["frequency"]);
            month.words[level] = [];
            temp["text"].forEach( d => {
                month.words[level].push(d);
            });
            month.words[level]
                .sort(function(a, b){//sort the terms by frequency
                return b.frequency-a.frequency});

            month.words[level] = month.words[level].slice(0, space);}
            else {
                delete month.words["LOW"];}
        });
        month.freq = freq;
    ...
    keys1 = ["CRITICAL","HIGH","MEDIUM"]
    data.forEach((month,i) => {
        keys1.forEach((topic,j) => {
